...
function loadState(): { setup: Setup | null; history: Session[] } {
  ...
  try {
    ...
    return { setup: parsed.setup ?? null, history: Array.isArray(parsed.history) ? parsed.history : [] };
  } catch { return { setup: null, history: [] }; }
}
...
/* FIX #1: Consistent dark theme input styles */
...
export default function Page() {
  ...
  const [activeTab, setActiveTab] = useState<'today' | 'history' | 'setup' | 'about'>('today');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f0f0f 0%, #1a1a1a 100%)', color: '#f5f5f5', fontFamily: '"Outfit", "SF Pro Display", -apple-system, BlinkMacSystemFont, sans-serif' }}>
      {/* FIX #3: Global styles for select options visibility in dark theme */}
      ...
      <header style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'flex', gap: 4 }}>
          {(['today', 'history', 'setup', 'about'] as const).map(tab => (
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
        {activeTab === 'setup' && <SetupView draftSetup={draftSetup} setDraftSetup={setDraftSetup} onSave={() => setStore({ setup: draftSetup, history })} onGenerate={generateTodayWorkout} hasSetup={!!setup} />}
        {activeTab === 'today' && <TodayView today={today} nextDayType={nextDayType} history={history} onGenerate={generateTodayWorkout} onUpdateLog={updateExerciseLog} onRegenerateWeights={regenerateWorkoutWeights} hasSetup={!!setup} />}
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 32 }}>
        <h2 style={{ margin: '0 0 24px', fontSize: 28, fontWeight: 700 }}>About FLEX</h2>
        
        <p style={{ fontSize: 16, lineHeight: 1.7, opacity: 0.9, marginBottom: 20 }}>
...
/* FIX #4: Proper TypeScript interfaces instead of `any` */
interface SetupViewProps { draftSetup: Setup; setDraftSetup: (setup: Setup) => void; onSave: () => void; onGenerate: (energy?: number, sleepHours?: number) => void; hasSetup: boolean; }
    ...
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 32 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20, marginBottom: 32 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 8, opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Goal</label>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
          <button onClick={onSave} style={{ flex: 1, padding: '14px 24px', background: 'linear-gradient(135deg, #fff 0%, #d0d0d0 100%)', border: 'none', borderRadius: 10, color: '#000', fontSize: 15, fontWeight: 700, cursor: 'pointer', transition: 'transform 0.2s' }}
...
interface TodayViewProps { today: Session | null; nextDayType: DayType; history: Session[]; onGenerate: (energy?: number, sleepHours?: number) => void; onUpdateLog: (exId: string, patch: Partial<ExerciseLog>) => void; onRegenerateWeights: (energy: number, difficulty: number, sleepHours?: number) => void; hasSetup: boolean; }
  ...
  if (!today) {
    return (
      <div style={{ maxWidth: 600, margin: '120px auto', textAlign: 'center' }}>
        ...
        <p style={{ opacity: 0.6, marginBottom: 32, fontSize: 15 }}>{hasSetup ? `Ready to start a ${nextDayType} workout?` : 'Complete your profile setup first, then generate your workout.'}</p>
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(100,200,255,0.1) 0%, rgba(150,100,255,0.1) 100%)', border: '1px solid rgba(100,200,255,0.2)', borderRadius: 16, padding: 24, marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            ...
            <p style={{ margin: '6px 0 0', opacity: 0.7, fontSize: 14 }}>{today.muscleGroups.join(' • ')}</p>
          </div>
          <div style={{ padding: '8px 16px', background: 'rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 13, fontWeight: 600 }}>~60 min</div>
        </div>
        {history.length > 1 && (
          <div style={{ padding: 16, background: 'rgba(0,0,0,0.2)', borderRadius: 10, fontSize: 13, lineHeight: 1.6, opacity: 0.9 }}>
...
function ProgressChart({ data }: { data: ReturnType<typeof extractProgressData> }) {
  if (data.length === 0) return <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 40, textAlign: 'center' }}><p style={{ opacity: 0.6 }}>No data available for selected filters</p></div>;
  ...
  const innerWidth = chartWidth - padding.left - padding.right, innerHeight = chartHeight - padding.top - padding.bottom;
  const allWeights = data.map(d => d.weight);
  const minWeight = Math.floor(Math.min(...allWeights) / 10) * 10 - 10, maxWeight = Math.ceil(Math.max(...allWeights) / 10) * 10 + 10;
  const liftColors: Record<LiftKey, string> = { bench: '#64c8ff', squat: '#ff6b9d', deadlift: '#ffd93d', ohp: '#95e1d3', row: '#c77dff' };
  const liftNames: Record<LiftKey, string> = { bench: 'Bench Press', squat: 'Squat', deadlift: 'Deadlift', ohp: 'Overhead Press', row: 'Barbell Row' };
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 24, overflow: 'hidden' }}>
      <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 20 }}>Weight Progress Over Time</h3>
      <div style={{ overflowX: 'auto' }}>
        <svg width={chartWidth} height={chartHeight} style={{ display: 'block' }}>
          {[0, 1, 2, 3, 4].map(i => { const weight = minWeight + (i / 4) * (maxWeight - minWeight); const y = yScale(weight); return (<g key={i}><line x1={padding.left} y1={y} x2={chartWidth - padding.right} y2={y} stroke="rgba(255,255,255,0.1)" strokeWidth={1} /><text x={padding.left - 10} y={y + 4} fill="rgba(255,255,255,0.5)" fontSize={12} textAnchor="end">{Math.round(weight)}</text></g>); })}
          ...
          {Object.entries(liftGroups).map(([lift, points]) => { if (points.length === 0) return null; const pathData = points.map((point, i) => { const x = xScale(data.indexOf(point), data.length); const y = yScale(point.weight); return i === 0 ? `M ${x} ${y}` : `L ${x} ${y}`; }).join(' '); return (<g key={lift}><path d={pathData} fill="none" stroke={liftColors[lift as LiftKey]} strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" />{points.map((point, i) => { const x = xScale(data.indexOf(point), data.length); const y = yScale(point.weight); return (<g key={i}><circle cx={x} cy={y} r={5} fill={liftColors[lift as LiftKey]} stroke="rgba(0,0,0,0.5)" strokeWidth={2} /><title>{`${point.liftName}: ${point.weight} lbs\n${point.date}\nEnergy: ${point.energy}/5`}</title></g>); })}</g>); })}
          {data.map((point, i) => { if (i % Math.max(1, Math.floor(data.length / 6)) === 0 || i === data.length - 1) { const x = xScale(i, data.length); return (<text key={i} x={x} y={chartHeight - padding.bottom + 20} fill="rgba(255,255,255,0.5)" fontSize={11} textAnchor="middle" transform={`rotate(-45, ${x}, ${chartHeight - padding.bottom + 20})`}>{point.date}</text>); } return null; })}
        </svg>
      </div>
      <div style={{ display: 'flex', gap: 20, marginTop: 24, flexWrap: 'wrap', justifyContent: 'center' }}>
        {Object.entries(liftGroups).filter(([, points]) => points.length > 0).map(([lift, points]) => (<div key={lift} style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ width: 16, height: 16, borderRadius: '50%', background: liftColors[lift as LiftKey] }} /><span style={{ fontSize: 13, fontWeight: 600 }}>{liftNames[lift as LiftKey]} ({points.length})</span></div>))}
      </div>
      <div style={{ marginTop: 24, padding: 16, background: 'rgba(0,0,0,0.2)', borderRadius: 10, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 16 }}>
        {Object.entries(liftGroups).filter(([, points]) => points.length > 0).map(([lift, points]) => { const weights = points.map(p => p.weight); const firstWeight = weights[0]; const lastWeight = weights[weights.length - 1]; const change = lastWeight - firstWeight; const percentChange = ((change / firstWeight) * 100).toFixed(1); return (<div key={lift} style={{ textAlign: 'center' }}><div style={{ fontSize: 11, opacity: 0.6, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{liftNames[lift as LiftKey]}</div><div style={{ fontSize: 20, fontWeight: 700, color: change >= 0 ? '#4ade80' : '#ff6b6b' }}>{change >= 0 ? '+' : ''}{change} lbs</div><div style={{ fontSize: 12, opacity: 0.7, marginTop: 2 }}>{change >= 0 ? '+' : ''}{percentChange}% gain</div></div>); })}
      </div>
    </div>
  );
}
...
function HistoryView({ history }: { history: Session[] }) {
  ...
  if (history.length === 0) return (<div style={{ textAlign: 'center', padding: '120px 0' }}><div style={{ fontSize: 48, marginBottom: 16 }}>📊</div><h2 style={{ fontSize: 24, fontWeight: 700, marginBottom: 12 }}>No History Yet</h2><p style={{ opacity: 0.6 }}>Your workout history will appear here once you start tracking.</p></div>);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 32, flexWrap: 'wrap' }}>
        <div><label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 8, opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Workout Type</label><select value={selectedDayType} onChange={(e) => setSelectedDayType(e.target.value as DayType | 'all')} style={selectStyle}><option value="all">All Workouts</option>{dayTypes.map(dt => (<option key={dt} value={dt}>{dt}</option>))}</select></div>
        <div><label style={{ display: 'block', fontSize: 12, fontWeight: 600, marginBottom: 8, opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Primary Lift</label><select value={selectedLift} onChange={(e) => setSelectedLift(e.target.value as LiftKey | 'all')} style={selectStyle}><option value="all">All Lifts</option><option value="bench">Bench Press</option><option value="squat">Squat</option><option value="deadlift">Deadlift</option><option value="ohp">Overhead Press</option><option value="row">Barbell Row</option></select></div>
      </div>
      {progressData.length > 0 && <div style={{ marginBottom: 32 }}><ProgressChart data={progressData} /></div>}
      <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 16, marginTop: 40 }}>Session Log</h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>{history.filter(s => selectedDayType === 'all' || s.dayType === selectedDayType).map((session, idx) => (<HistoryCard key={session.id} session={session} isRecent={idx === 0} selectedLift={selectedLift} />))}</div>
    </div>
  );
}
...
function InputField({ label, value, onChange, type = 'text' }: InputFieldProps) {
  return (<label style={{ display: 'grid', gap: 6 }}><span style={{ fontSize: 12, fontWeight: 600, opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</span><input type={type} value={value} onChange={(e) => onChange(e.target.value)} style={darkInputStyle} /></label>);
}
...
function ExerciseCard({ exercise, log, onUpdateLog }: ExerciseCardProps) {
  ...
  return (
    <div style={{ background: isPrimary ? 'rgba(100,200,255,0.05)' : 'rgba(255,255,255,0.03)', border: isPrimary ? '1px solid rgba(100,200,255,0.15)' : '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 20, transition: 'all 0.2s' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div style={{ flex: 1 }}><h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: isPrimary ? '#64c8ff' : '#fff' }}>{exercise.name}</h3><p style={{ margin: '4px 0 0', fontSize: 13, opacity: 0.6 }}>{exercise.muscleGroups.join(', ')}</p></div>
        <div style={{ padding: '6px 12px', background: 'rgba(0,0,0,0.2)', borderRadius: 6, fontSize: 13, fontWeight: 600 }}>{exercise.sets} × {exercise.reps}</div>
      </div>
      {exercise.targetWeightLb && <div style={{ padding: '10px 14px', background: 'rgba(0,0,0,0.2)', borderRadius: 8, marginBottom: 16, fontSize: 14, fontWeight: 600 }}>Target: <span style={{ color: '#64c8ff' }}>{exercise.targetWeightLb} lb</span></div>}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12 }}>
        <div><label style={{ display: 'block', fontSize: 11, opacity: 0.6, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Weight (lb)</label><input type="number" value={log?.actualWeightLb ?? ''} onChange={(e) => onUpdateLog({ actualWeightLb: e.target.value === '' ? undefined : Number(e.target.value) })} placeholder={exercise.targetWeightLb?.toString() || '—'} style={inputStyle} /></div>
        <div><label style={{ display: 'block', fontSize: 11, opacity: 0.6, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Reps (10,9,8...)</label><input type="text" value={log?.actualReps ?? ''} onChange={(e) => onUpdateLog({ actualReps: e.target.value })} placeholder="10,9,8" style={inputStyle} /></div>
        <div><label style={{ display: 'block', fontSize: 11, opacity: 0.6, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>RPE (1-10)</label><input type="number" min={1} max={10} value={log?.rpe ?? ''} onChange={(e) => onUpdateLog({ rpe: e.target.value === '' ? undefined : clamp(Number(e.target.value), 1, 10) })} placeholder="7" style={inputStyle} /></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background: isRecent ? 'rgba(100,200,255,0.05)' : 'rgba(255,255,255,0.03)', border: isRecent ? '1px solid rgba(100,200,255,0.15)' : '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
      <div onClick={() => setExpanded(!expanded)} style={{ padding: 20, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 6 }}><h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{session.dayType}</h3>{isRecent && <span style={{ padding: '4px 10px', background: 'rgba(100,200,255,0.2)', borderRadius: 6, fontSize: 11, fontWeight: 700, color: '#64c8ff', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Latest</span>}</div>
          <div style={{ display: 'flex', gap: 16, fontSize: 13, opacity: 0.6, flexWrap: 'wrap' }}><span>{formatDate(session.dateISO)}</span><span>•</span><span>Energy: {session.energy}/5</span><span>•</span><span>Difficulty: {session.difficulty}/5</span>{session.sleepHours && (<><span>•</span><span>Sleep: {session.sleepHours}h</span></>)}</div>
        </div>
        <div style={{ fontSize: 20, opacity: 0.5, transform: expanded ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}>▼</div>
      </div>
      {expanded && (
        <div style={{ padding: '0 20px 20px', borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: -10, paddingTop: 20 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {session.workout.map(ex => { const isPrimary = ex.primary !== 'accessory'; const isSelected = selectedLift === 'all' || ex.primary === selectedLift; return (<div key={ex.id} style={{ padding: 14, background: isSelected && isPrimary ? 'rgba(100,200,255,0.1)' : 'rgba(0,0,0,0.2)', border: isSelected && isPrimary ? '1px solid rgba(100,200,255,0.3)' : '1px solid transparent', borderRadius: 8, fontSize: 14 }}><div style={{ fontWeight: 600, marginBottom: 4 }}>{ex.name}</div><div style={{ opacity: 0.6, fontSize: 12 }}>{ex.sets} × {ex.reps}{ex.targetWeightLb && <span style={{ marginLeft: 8, color: isSelected && isPrimary ? '#64c8ff' : 'inherit', fontWeight: isSelected && isPrimary ? 700 : 400 }}>@ {ex.targetWeightLb} lb</span>}</div></div>); })}
          </div>
        </div>
      )}
    </div>
